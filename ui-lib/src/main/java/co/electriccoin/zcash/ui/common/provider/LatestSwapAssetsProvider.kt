...
data class SimpleSwapAsset(val tokenTicker: String, val chainTicker: String)
...
class LatestSwapAssetsProviderImpl(
    ...
    private fun Set<String>.toSimpleAssetSet() = this
        .map {
            val data = it.split(":")
            val order = data[0]
            order to SimpleSwapAsset(
                tokenTicker = data[1],
                chainTicker = data[2]
            )
        }
